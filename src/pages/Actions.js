    ...
    <div style={{maxWidth:"540px",margin:"0 auto",padding:"32px 20px"}}>
      ...
      <div style={{
        ...
        <div style={{display:"flex",alignItems:"center",gap:"10px",marginBottom:"16px"}}>
          ...
          <p style={{fontSize:"15px",fontWeight:"700",color:"#fca5a5"}}>Immediate Actions</p>
        </div>
        {(result.immediateActions || []).map((action, i) => (
          <div key={i} style={{
            display:"flex",gap:"12px",padding:"12px 0",
            borderBottom: i < (result.immediateActions.length-1) ? "1px solid rgba(239,68,68,0.1)" : "none"
          }}>
            <div style={{
            ...
            <p style={{fontSize:"14px",color:"rgba(255,255,255,0.8)",lineHeight:"1.6"}}>{action}</p>
          </div>
        ))}
      ...
      <div style={{
        ...
        <div style={{display:"flex",alignItems:"center",gap:"10px",marginBottom:"16px"}}>
          ...
          <p style={{fontSize:"15px",fontWeight:"700",color:"#86efac"}}>Prevention Tips</p>
        </div>
        {(result.preventionTips || []).map((tip, i) => (
          <div key={i} style={{
            display:"flex",gap:"12px",padding:"12px 0",
            borderBottom: i < (result.preventionTips.length-1) ? "1px solid rgba(34,197,94,0.1)" : "none"
          }}>
            <span style={{color:"#4ade80",flexShrink:0,fontSize:"16px"}}>✓</span>
            <p style={{fontSize:"14px",color:"rgba(255,255,255,0.8)",lineHeight:"1.6"}}>{tip}</p>
          </div>
        ))}
      </div>
      ...
      }}>Scan Another Message</button>

      {showAlert && (
        <FamilyAlertPopup result={result} onClose={() => setShowAlert(false)} />
      )}
    </div>
  );
}
